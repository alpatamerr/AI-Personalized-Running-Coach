...
const getUserBaseline = async (userId) => {
  // Try user data
  let runs = await pool.query('SELECT * FROM run_data WHERE user_id = $1 ORDER BY run_date DESC LIMIT 8', [userId]);
  if (!runs.rows.length) {
    // Fallback: demo Strava user
    const demo = await pool.query("SELECT id FROM users WHERE is_strava_user = true LIMIT 1");
    if (demo.rows.length) {
      runs = await pool.query('SELECT * FROM run_data WHERE user_id = $1 ORDER BY run_date DESC LIMIT 8', [demo.rows[0].id]);
    }
  }
  return runs.rows;
};
...
// --- Generate Training Plan using latest goal and wizard answers ---
exports.generateTrainingPlan = async (req, res) => {
  try {
    const { userId } = req.body;
    if (!userId) return res.status(400).json({ success: false, error: "userId required" });

    // 1. Get latest goal (should include days and weekly_km!)
    const { rows: goals } = await pool.query(
      `SELECT * FROM goals WHERE user_id = $1 ORDER BY updated_at DESC LIMIT 1`, [userId]
    );
    if (!goals.length) return res.status(404).json({ success: false, error: 'No goal found for user' });

    const goal = goals[0];
    // Parse days and weekly_km from goal
    ...
    // 2. Get baseline run_data (user or fallback demo)
    const runs = await getUserBaseline(userId);
    // Use average pace as baseline, or 6.0 min/km if no data
    const avgPace = runs.length ? runs.reduce((a, r) => a + Number(r.pace), 0) / runs.length : 6.0;

    // 3. Generate 8-week plan using user's answers
    const plan = [];
    const planLengthWeeks = 8;
    for (let week = 1; week <= planLengthWeeks; ++week) {
      runDays.forEach(day => {
        plan.push({
          week,
          day,
          type: "Run",
          distance: (weeklyDistance / runDays.length).toFixed(1),
          target_pace: (avgPace - 0.01 * week).toFixed(2)
        });
      });
    }
    ...
    res.status(500).json({ success: false, error: "Failed to generate plan", details: err.message });
  }
};


// --- Dashboard stats: Always handle no running data safely ---
exports.getDashboardStats = async (req, res) => {
  try {
    const { userId } = req.query;
    if (!userId) return res.status(400).json({ error: "userId required" });
    const { rows } = await pool.query(
      `SELECT run_date, distance_km, pace, average_heartrate, average_cadence FROM run_data WHERE user_id = $1 ORDER BY run_date DESC LIMIT 56`,
      [userId]
    );
    ...
    for (let i = 0; i < 8; ++i) {
      const weekRows = rows.slice(i * 7, (i + 1) * 7);
      if (weekRows.length === 0) break;
      const total_km = weekRows.reduce((a, r) => a + (Number(r.distance_km) || 0), 0);
      const avg_pace = weekRows.length ? weekRows.reduce((a, r) => a + (Number(r.pace) || 0), 0) / weekRows.length : null;
      const avg_hr = weekRows.length ? weekRows.reduce((a, r) => a + (r.average_heartrate ? Number(r.average_heartrate) : 0), 0) / weekRows.length : null;
      const avg_cadence = weekRows.length ? weekRows.reduce((a, r) => a + (r.average_cadence ? Number(r.average_cadence) : 0), 0) / weekRows.length : null;
      weeks.push({
        week: `Week ${i + 1}`,
        total_km: total_km.toFixed(2),
        avg_pace: avg_pace ? avg_pace.toFixed(2) : null,
    ...
    const total_km = rows.reduce((a, r) => a + (Number(r.distance_km) || 0), 0);
    const avg_pace = rows.length ? rows.reduce((a, r) => a + (Number(r.pace) || 0), 0) / rows.length : null;
    const avg_hr = rows.length ? rows.reduce((a, r) => a + (r.average_heartrate ? Number(r.average_heartrate) : 0), 0) / rows.length : null;
    const avg_cadence = rows.length ? rows.reduce((a, r) => a + (r.average_cadence ? Number(r.average_cadence) : 0), 0) / rows.length : null;
    res.json({
      total_km: total_km.toFixed(2),
      avg_pace: avg_pace ? avg_pace.toFixed(2) : null,
      avg_hr: avg_hr ? Math.round(avg_hr) : null,
